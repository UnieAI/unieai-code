//! Custom CA handling Shared by UnieAI outbound HTTP and websocket clients.
//!
//! UnieAI constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because UnieAI itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// Builds a reqwest client that honors UnieAI custom CA environment variables.
///
...
/// easy mistake to make when adding a new outbound UnieAI HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a UnieAI custom CA bundle is configured.
///
...
/// Builds a rustls client config using native roots and any configured UnieAI custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the UnieAI custom
...
trait EnvSource {
    ...
    ///
    /// `CODEX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the UnieAI-specific override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for UnieAI CA overrides.
    ///
    /// This accepts a few common real-world variants so UnieAI behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// UnieAI only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
