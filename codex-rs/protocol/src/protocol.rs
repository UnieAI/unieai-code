//! Defines the protocol for UnieAI session between a client and an agent.
//!
...
pub struct ConversationStartParams {
    /// Whether UnieAI response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route UnieAI when the session ends.
    ...
    /// Sends automatic UnieAI responses as realtime conversation items instead of handoff appends.
    pub codex_responses_as_items: bool,
    /// Optional prefix added to automatic UnieAI response items when `codex_responses_as_items` is set.
    pub codex_response_item_prefix: Option<String>,
    /// Selects how automatic UnieAI handoffs are routed in Frameless Bidi sessions.
    ...
    /// Whether to append UnieAI's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request UnieAI to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by UnieAI.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// UnieAI errors that we expose to clients.
...
mod tests {
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".unieai")).expect("create .unieai");
        let canonical_cwd = codex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_unieai = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".unieai"))
            .expect("canonical .unieai");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_unieai.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".codex"))
            .expect("canonical .codex");
        let expected_dot_unieai =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".unieai"))
                .expect("canonical .unieai");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
            ...
            vec![
                (
                    ...
                    vec![
                        ...
                        expected_dot_unieai.to_path_buf(),
