...
pub(super) fn render_human_report(report: &DoctorReport, options: HumanOutputOptions) -> String {
    ...
    let _ = writeln!(
        out,
        "{} {}",
        bold("UnieAI Code Doctor", options),
...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run unieai doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
mod tests {
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
UnieAI Code Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `unieai login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `unieai login`.
-------------------------------------------------------------
