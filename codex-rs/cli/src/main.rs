...
enum Subcommand {
    ...
    /// Manage external MCP servers for UnieAI.
    Mcp(McpCli),

    /// Manage UnieAI plugins.
    Plugin(PluginCli),

    /// Start UnieAI as an MCP server (stdio).
    ...
    /// Update UnieAI to the latest version.
    Update,

    /// Diagnose local UnieAI installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a UnieAI-provided sandbox.
    ...
    /// Apply the latest diff produced by UnieAI agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from UnieAI Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of UnieAI.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for UnieAI tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating UnieAI via `{cmd_str}`...");
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`unieai update` is not available in debug builds. Install a release build of UnieAI to use this command."
        );
    }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `unieai mcp`: `unieai`, `unieai exec`, `unieai review`, `unieai resume`, `unieai archive`, `unieai delete`, `unieai unarchive`, `unieai fork`, `unieai mcp`, `unieai sandbox`, and `unieai debug prompt-input`."
        ),
    }
}
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged UnieAI local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
/// Build the final `TuiCli` for a `unieai resume` invocation.
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run unieai resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run unieai resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
