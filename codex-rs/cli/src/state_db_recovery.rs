...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("UnieAI Code rebuilt its local database.");
    eprintln!(
        "UnieAI Code detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("UnieAI Code couldn't start because its local database appears to be damaged.");
    eprintln!("Run `unieai doctor` to check your setup and get next-step guidance.");
