...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed UnieAI binary {}",
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://www.unieai.com/code/install.sh")
        .await
        .context("failed to fetch standalone UnieAI Code updater")?
        .error_for_status()
        .context("standalone UnieAI Code updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone UnieAI Code updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone UnieAI Code updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone UnieAI Code updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone UnieAI Code updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone UnieAI Code updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone UnieAI Code updater exited with status {status}")
    }
}
