...
#[command(
    version,
    override_usage = "unieai exec [OPTIONS] [PROMPT]\n       unieai exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Error out when config.toml contains fields that are not recognized by this version of UnieAI Code.
    ...
    /// Allow running UnieAI Code outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "unieai resume --last <prompt>" treats the positional as a prompt, not a session id.
