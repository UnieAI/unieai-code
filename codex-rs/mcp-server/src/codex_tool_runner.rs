//! Asynchronous worker that executes a **UnieAI** tool-call inside a spawned
...
/// To adhere to MCP `tools/call` response format, include the UnieAI
...
/// Run a complete UnieAI session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start UnieAI session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the UnieAI submission so that
