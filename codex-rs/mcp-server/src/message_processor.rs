...
impl MessageProcessor {
    ...
    async fn handle_initialize(
        ...
        // Preserve UnieAI's existing non-spec `serverInfo.user_agent` field.
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "codex" => self.handle_tool_call_unieai(id, arguments).await,
            "codex-reply" => {
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load UnieAI configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for UnieAI tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
        ...
        // Spawn an async task to handle the UnieAI session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the UnieAI session and stream events back to the client.
    ...
    async fn handle_tool_call_codex_session_reply(
        ...
        let codex_tool_call_reply_param: CodexToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse UnieAI tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for UnieAI tool: {e}"
                    ))]);
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the UnieAI thread from the server.
        ...
        if let Err(e) = codex_arc
            ...
            .await
        {
            tracing::error!("Failed to submit interrupt to UnieAI: {e}");
            return;
        }
