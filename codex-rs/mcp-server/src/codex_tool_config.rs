...
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the UnieAI conversation.
    pub prompt: String,
...
/// Builds a `Tool` definition (JSON schema etc.) for the UnieAI tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "UnieAI tool schema should serialize");

    Tool::new(
        "codex",
        "Run a UnieAI session. Accepts configuration parameters matching the UnieAI Config struct.",
        input_schema,
    )
    .with_title("Codex")
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the UnieAI conversation and the
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this UnieAI session.
    ...
    /// The *next user prompt to continue the UnieAI conversation.
    pub prompt: String,
}
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    ...
    let input_schema =
        create_tool_input_schema(schema, "UnieAI reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a UnieAI conversation by providing the thread id and prompt.",
        input_schema,
    )
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a UnieAI session. Accepts configuration parameters matching the UnieAI Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the UnieAI conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a UnieAI conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt to continue the UnieAI conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this UnieAI session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
