...
pub enum Command {
    /// Submit a new UnieAI Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a UnieAI Cloud task.
    Status(StatusCommand),
    /// List UnieAI Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a UnieAI Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a UnieAI Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in UnieAI Cloud.
    ...
    /// Git branch to run in UnieAI Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// UnieAI Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// UnieAI Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// UnieAI Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
