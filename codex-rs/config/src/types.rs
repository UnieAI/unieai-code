//! Types used to define loaded and effective UnieAI configuration values.
...
pub enum ResumeCwdMode {
    /// Use the directory where UnieAI was launched.
    Current,
...
/// Determine where UnieAI should store CLI auth credentials.
...
/// Determine where UnieAI should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across UnieAI product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across UnieAI product surfaces.
    pub enabled: Option<bool>,
    /// When set, feedback is POSTed as JSON to this URL (e.g. a Google Apps
    /// Script web app) instead of the built-in Sentry upload.
    pub webhook_url: Option<String>,
    /// Optional shared secret sent as `token` in the webhook JSON body so the
    /// receiver can reject unauthorized writes.
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in UnieAI rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, UnieAI does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when UnieAI cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// transcript exceeds this cap. Omit to use UnieAI's terminal-specific default. Set to `0` to
...
/// (primarily the UnieAI IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of UnieAI-managed fast defaults.
...
pub struct PluginMcpServerConfig {
    /// When `false`, UnieAI skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time UnieAI successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision UnieAI last successfully activated for this marketplace.
    #[serde(default)]
