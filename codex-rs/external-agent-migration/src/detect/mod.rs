...
impl ExternalAgentConfigService {
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            ...
            |repo_root| codex_config::project_config_dir(repo_root).join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            ...
            |repo_root| codex_config::project_config_dir(repo_root).join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            ...
            |repo_root| codex_config::project_config_dir(repo_root).join("agents"),
        );
