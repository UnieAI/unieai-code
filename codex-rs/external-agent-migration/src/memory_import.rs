...
- For each project, always read its source `MEMORY.md` first when it exists. Use it to seed or update that project's scoped entry in UnieAI `MEMORY.md`, and add only the smallest broadly useful route to `memory_summary.md`.
...
- Keep source-specific frontmatter in the imported resource. Do not reinterpret fields such as `metadata.originSessionId` as a UnieAI `thread_id`, `rollout_path`, or `updated_at`.
...
pub(super) async fn import(
    ...
    let state_db = state_db.ok_or_else(|| {
        io::Error::new(
            ...
            "memory import requires the UnieAI state database",
        )
    })?;
