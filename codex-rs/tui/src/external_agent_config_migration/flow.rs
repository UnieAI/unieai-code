...
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_DAEMON_UNAVAILABLE_MESSAGE: &str = "Import from other apps is unavailable while UnieAI Code is connected to the local app-server daemon. Restart with `unieai --no-daemon` and run /import.";
