...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in UnieAI Code Desktop.";
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in UnieAI Code Desktop: {err}. Install or launch UnieAI Code Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch UnieAI Code Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch UnieAI Code Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("UnieAI Code Desktop is only available on macOS and Windows".to_string())
}
