...
async fn connector_auth(config: &Config) -> anyhow::Result<CodexAuth> {
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require UnieAI backend auth"
    );
    Ok(auth)
}
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_codex_backend(),
        "ChatGPT backend requests require UnieAI backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run unieai login"
    );
