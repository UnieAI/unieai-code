...
pub async fn load_default_config_for_test_with_cloud_config_bundle(
    ...
    // The session mesh is on by default in UnieAI Code. Tests that are not
    // about it must not bind machine-local sockets or see its two tools in
    // the model's tool list; tests that are enable it explicitly.
    let _ = config
        .features
        .disable(codex_features::Feature::SessionMesh);
    config
}
