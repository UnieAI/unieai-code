...
//!
//! `ModelClient` is intended to live for the lifetime of a UnieAI session and holds the stable
...
///
/// This holds configuration and state that should be shared across turns within a UnieAI session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each UnieAI turn. Reusing it across turns would replay
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a UnieAI session. Per-turn values
...
impl ModelClientSession {
    ...
    /// Permanently disables WebSockets for this UnieAI session and resets WebSocket state.
    ///
