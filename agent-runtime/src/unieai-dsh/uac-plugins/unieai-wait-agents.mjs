...
export function createChildRegistry() {
  ...
  return {
    started(parent, info) {
      childrenOf(parent).set(String(info.id), { id: String(info.id), running: true, stopReason: null, output: "", endedAt: null });
      wake(parent);
    },
    ended(parent, info) {
...
export async function waitForChildren(registry, parent, args, { cfg = DEFAULTS, signal } = {}) {
  ...
  // Without ids: the children running now. Children that already finished
  // before this call are reported, not waited on.
  const targets = wanted ?? new Set(known().filter((child) => child.running).map((child) => child.id));
  ...
  try {
    while (!satisfied() && !stop.aborted) await registry.nextChange(parent, stop);
  } finally {
...
export function apply(ctx, config = {}) {
  ...
  ctx.on("agent/disposed", ({ agent }) => {
    ...
    for (const [child, parent] of known) if (parent === agent.id || child === agent.id) known.delete(child);
  });
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        return waitForChildren(registry, exec.agent.id, args, { cfg, signal: exec.signal });
      },
    }),
  );
}
